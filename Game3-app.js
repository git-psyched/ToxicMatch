...
function ToxicMatch() {
  ...
  if (gameState === 'menu') {
    return e('div', {className: 'flex flex-col items-center justify-center h-screen bg-white p-6'},
      e('h1', {className: 'text-6xl font-bold text-slate-900 mb-8 tracking-tighter'}, 'ToxicMatch'),
  ...
  return e('div', {className: 'h-screen flex flex-col bg-slate-50'},
    e('div', {className: 'p-4 bg-white border-b flex justify-between items-center shadow-sm'},
      ...
      e('span', {className: 'text-red-600 font-bold'}, '❤️ '.repeat(Math.max(0, lives)))
    ),
      ...
      fallingMeds.map(m => 
        e('div', {
          ...
          style: { 
            ...
            animation: lastMistakeId === m.id ? 'shake 0.4s' : 'none'
          },
          className: 'bg-white border-2 border-slate-900 px-4 py-2 shadow-xl font-bold text-lg falling-pill'
        }, m.name)
      )
    ),
    e('div', {className: 'p-2 bg-slate-200 border-t grid gap-2 pb-8'},
      Object.entries(SIDE_EFFECT_GROUPS).map(([group, effects]) => 
        e('div', {key: group},
          e('p', {className: 'text-[10px] uppercase font-bold text-slate-500 mb-1 px-1'}, group),
          e('div', {className: 'grid grid-cols-4 gap-1'},
            effects.map(eff => 
              e('button', {
                key: eff,
                onClick: () => handleMatch(eff),
                className: 'bg-white border border-slate-300 py-3 text-[9px] font-bold leading-none hover:bg-slate-800 hover:text-white rounded active:scale-95 transition-all'
              }, eff)
            )
          )
        )
